<script lang="ts">
  ...
  function updateChart() {
    const labels = Array.from({ length: 10 }, (_, i) => `Iteration ${i + 1}`);
    
    const datasets = results.map(result => ({
      label: result.algorithm,
      data: result.sign_times_ns?.slice(0, 10).map(t => t / 1000000) || [],
...
<div class="bg-cat-mantle p-4 rounded-lg shadow-lg">
  <h3 class="text-lg font-semibold mb-4 text-cat-text">Performance Over Time</h3>
  <div class="h-64">
